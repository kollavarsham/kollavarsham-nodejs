...
var calendar = {
  ...
  aharganaToKali            : function (ahargana) {
    return math.truncate(ahargana * celestial.YugaRotation.sun / celestial.Yuga.CivilDays);
  },
  kaliToAhargana            : function (yearKali, masaNum, tithiDay) {
    var sauraMasas = yearKali * 12 + masaNum; // expired saura masas

    var adhiMasas = math.truncate(sauraMasas * celestial.Yuga.Adhimasa / ( 12 * celestial.YugaRotation.sun ));  // expired adhimasas

    var candaMasas = sauraMasas + adhiMasas;  // expired candra masas

    var tithis = 30 * candaMasas + tithiDay - 1; // expired tithis

    var avamas = math.truncate(tithis * celestial.Yuga.Ksayadina / celestial.Yuga.Tithi); // expired avamas
  ...
  findSamkranti             : function (leftAhargana, rightAhargana) {
    // TODO: Remove this below redundant block
