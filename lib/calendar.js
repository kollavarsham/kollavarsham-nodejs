...
var calendar = {
  ...
  aharganaToKali            : function (ahargana) {
    return math.truncate(ahargana * celestial.planetarySystem.planets.sun.YugaRotation / celestial.planetarySystem.yuga.CivilDays);
  },
  kaliToAhargana            : function (yearKali, masaNum, tithiDay) {
    var sauraMasas = yearKali * 12 + masaNum; // expired saura masas

    var adhiMasas = math.truncate(sauraMasas * celestial.planetarySystem.yuga.Adhimasa / ( 12 * celestial.planetarySystem.planets.sun.YugaRotation ));  // expired adhimasas

    var candaMasas = sauraMasas + adhiMasas;  // expired candra masas

    var tithis = 30 * candaMasas + tithiDay - 1; // expired tithis

    var avamas = math.truncate(tithis * celestial.planetarySystem.yuga.Ksayadina / celestial.planetarySystem.yuga.Tithi); // expired avamas
  ...
  julianDayToMlWeekday      : function (julianDay) {
  ...
  calculateSamkranti        : function (ahargana, desantara) {
  ...
  getMlMalayalaMasaName     : function (number) {
  ...
  getMlMalayalaNaksatraName : function (tllong) {
